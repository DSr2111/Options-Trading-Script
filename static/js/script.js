...
// Apply conditional formatting and format PoP after the table is loaded
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (table) {
    ...
    // Collect all PoP values and format them
    rows.forEach((row) => {
      ...
      popCell.textContent = `${popValue.toFixed(2)}%`; // Add % symbol
    });

    // Update PoP header to indicate percentage
    const popHeader = table.querySelector('thead th:nth-child(11)');
    popHeader.textContent = 'PoP (%)';
